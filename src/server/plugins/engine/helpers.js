...
export function proceed(request, h, nextUrl) {
  ...
  const isReturnAllowed = payload && 'action' in payload ? payload.action === FormAction.Continue || payload.action === FormAction.Validate : false;

  // Redirect to return location (optional)
  const response = isReturnAllowed && isPathRelative(returnUrl) ? h.redirect(returnUrl) : h.redirect(redirectPath(nextUrl));

  // Redirect POST to GET to avoid resubmission
  return method === 'post' ? response.code(StatusCodes.SEE_OTHER) : response.code(StatusCodes.MOVED_TEMPORARILY);
}

/**
 * Encodes a URL, returning undefined if the process fails.
 */
export function encodeUrl(link) {
  if (link) {
    try {
...
export function getPageHref(page, pathOrQuery, queryOnly = {}) {
 ...
 */
export function redirectPath(nextUrl, query = {}) {
  const isRelative = isPathRelative(nextUrl);

  // Filter string query params only
  const params = Object.entries(query).filter(query => typeof query[1] === 'string');

  // Build URL with relative path support
  const url = isRelative ? new URL(nextUrl, 'http://example.com') : new URL(nextUrl);
...
export function normalisePath(path = '') {
  return path.trim() // Trim empty spaces
  .replace(/^\//, '') // Remove leading slash
  .replace(/\/$/, ''); // Remove trailing slash
}
export function getPage(model, request) {
  const {
    params
  } = request;
...
export function findPage(model, path) {
...
export function checkFormStatus(path) {
...
export function checkEmailAddressForLiveFormSubmission(emailAddress, isPreview) {
  if (!emailAddress && !isPreview) {
    throw Boom.internal('An email address is required to complete the form submission');
  }
}
...
export function getError(detail) {
  const {
    context,
    message,
    path
  ...
  const href = `#${name}`;
  const text = upperFirst(message.replace(/\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d\.\d+([+-][0-2]\d:[0-5]\d|Z)/, text => format(parseISO(text), 'd MMMM yyyy')));
  return {
    path,
    href,
    name,
