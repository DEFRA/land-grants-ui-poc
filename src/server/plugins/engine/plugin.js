  ...
  register(server, options) {
    ...
    const loadFormPreHandler = async (request, h) => {
      ...
      if (!item || !isEqual(item.updatedAt, state.updatedAt)) {
        ...
        const basePath = isPreview ? `${PREVIEW_PATH_PREFIX.substring(1)}/${formState}/${slug}` : slug;
    ...
    const getHandler = (request, h) => {
      ...
      return redirectOrMakeHandler(request, h, (page, context) => page.makeGetRouteHandler()(request, context, h));
    };
