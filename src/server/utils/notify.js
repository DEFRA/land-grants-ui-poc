...
const apiKeyId = notifyAPIKey?.substring(notifyAPIKey.length - 36, notifyAPIKey.length);
const serviceId = notifyAPIKey?.substring(notifyAPIKey.length - 73, notifyAPIKey.length - 37);
...
export async function sendNotification(args) {
  ...
  return postJson('https://api.notifications.service.gov.uk/v2/notifications/email', {
    payload: {
